...
fn main() {
    ...
    if let Err(err) = check_data(&mut num_cols, &mut num_rows, &mut puzzle) {
...
fn check_data(num_cols: &mut usize, num_rows: &mut usize, puzzle: &mut Vec<StringRecord>) -> Result<(), Box<dyn Error>> {
...
fn compute_headers(col_headers: &mut Vec<Vec<usize>>, row_headers: &mut Vec<Vec<usize>>, num_cols: usize, num_rows: usize, puzzle: &Vec<StringRecord>) {
    for c in 0..num_cols {
...
fn solve_line(line: Vec<&str>, nums: Vec<usize>) -> Vec<&str> {
...
fn get_extreme(line: Vec<&str>, nums: Vec<usize>, dir: i8) -> Vec<usize> {
...
impl Possibility for Square<'_> {
    fn new() -> Square<'static> {
        return Square { could_be_filled: "", could_be_empty: "" };
    }
